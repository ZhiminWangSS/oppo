...
class UnityEnvironment(BaseUnityEnvironment):
	...
	def get_action_space(self):
		...
		for agent_id in range(self.num_agents):
			...
			# TODO:  Now the action space is base on graph
				...
				obs_type = 'partial'
			visible_graph = self.get_observation(agent_id, obs_type)##{"messages":  ,  "edges":   ,"nodes":   ,"location":}
			dict_action_space[agent_id] = [node['id'] for node in visible_graph['nodes']]## the structure
		return dict_action_space

	def check_subgoal(self, subgoal):##important function for capo
		# Check if the subgoal is satisfied
		# If the subgoal is satisfied, return True
		# If the subgoal is not satisfied, return False
		if subgoal is None: return False  # how to use
	...
	def get_goal(self, task_spec, agent_goal):##how it works?
		if agent_goal == 'full':
		...
		elif agent_goal == 'LLM':  #unchanged agent_goal
			'''
			...
			new_task_goal = {}
			for predicate, count in task_spec.items():#{'on_pudding_268': 1, 'on_juice_268': 1, 'on_apple_268': 1, 'on_cupcake_268': 2}
				if count == 0:
					continue
				rel, obj1, obj2 = predicate.split('_')
			...
			# res_dict.update(predicates_grab)
			#{'on_pudding_<coffeetable> (268)': [1, True, 2], 'on_juice_<coffeetable> (268)': [1, True, 2], 'on_apple_<coffeetable> (268)': [1, True, 2], 'on_cupcake_<coffeetable> (268)': [2, True, 2]}
			return res_dict
		else:
			raise NotImplementedError
	...
	def all_detection_id(self) -> list:
		return [node['id'] for node in self.full_graph['nodes'] if node['class_name'] in self.detection_all_object]#structure
...
#		obs = self.get_observations()
	...
	def get_observations(self):##carefully reading 
		curr_graph = self.get_graph()
		curr_graph = utils.inside_not_trans(curr_graph)
		self.full_graph = curr_graph#{"nodes":[{'id': 1, 'category': 'Characters', 'class_name': 'character', 'prefab_name': 'Female1', 'obj_transform': {'position': [...], 'rotation': [...], 'scale': [...]}, 'bounding_box': {'center': [...], 'size': [...]}, 'properties': [], 'states': []}...],"edges":[{'from_id': 375, 'to_id': 11, 'relation_type': 'INSIDE'}...]}
		...
		for agent_id in range(self.num_agents):
			...
			dict_observations[agent_id]["finish_goal"] = self.finish_goal
			self.finish_goal = 0
