...
class ArenaMP(object):
    ...
    def reset(self, task_id=None):
        ...
        for it, agent in enumerate(self.agents):
            if 'LLM_vision' in agent.agent_type:
                ...
                episode_logger = self.init_episode_logs(self.record_dir, str(task_id)+f"agent{it}")## add when shaokang debug
    ...
    def get_actions(self, obs, action_space=None, true_graph=False):
        ...
        for it, agent in enumerate(self.agents):
            ...
            elif 'LLM_roco' in agent.agent_type:
                dict_actions[it], dict_info[it] = agent.get_action_roco(obs[it], goal_spec)
