...
class ArenaMP(object):
    ...
    def reset(self, task_id=None):
        ...
        agent_init_rooms = [None,None,None,None]
        for it, agent in enumerate(self.agents):
            if 'LLM_vision' in agent.agent_type:
                ...
                agent_init_rooms[agent.agent_id] = agent.current_room
            else:
                agent.reset(self.env.full_graph)
        for it, agent in enumerate(self.agents):
            for agent_id,agent_init_room in enumerate(agent_init_rooms): 
        ...
        for it, agent in enumerate(self.agents):
            ...
            if agent.agent_type in ['MCTS', 'Random', 'MCTS_vision']:
                ...
                if dict_actions[it].startswith('[send_message]'):
                    for id,obs_single in enumerate(obs):
                        if id == it:
                            continue
                        obs[id]['messages'][it] = dict_actions[it][14:]
