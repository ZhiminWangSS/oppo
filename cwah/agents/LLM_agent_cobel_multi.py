...
class LLM_agent_cobel:
    ...
    def get_action_cobel(self, observation, goal):
        ...
        if self.communication:
            for i in range(len(observation["messages"])):
                if observation["messages"][i] is not None:
                    ...
                    self.dialogue_history.append(f"{self.agent_names[i]}: {observation['messages'][i]}")
        ...
        for e in obs['edges']:
            ...
            if x in range(self.agent_num+1) and x!= self.agent_id+1: 
                if r == 'INSIDE':
                    # print("oppoid",x)
    ...
    def parse_belief_line_con(self,belief_type,beliefs):
        print("======start analyse the belief============")
