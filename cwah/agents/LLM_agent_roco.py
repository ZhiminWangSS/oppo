...
from cwah.LLM.LLM_roco import LLM_roco
import re
import copy
...
class LLM_agent:
	...
	def __init__(self, agent_id, char_index, args):
		...
		self.cot = args.cot
		self.args = args
		self.LLM = LLM_roco(self.source, self.lm_id, self.prompt_template_path, self.communication, self.cot, self.args, self.agent_id)
		self.action_history = []
	...
	def LLM_disscuss(self,max_call = None):
		output = self.LLM.disscuss(self.current_room,[self.id2node[x] for x in self.grabbed_objects], self.satisfied, self.unchecked_containers, self.ungrabbed_objects, self.id_inside_room[self.goal_location_id], self.action_history, self.dialogue_history, self.opponent_grabbed_objects, self.id_inside_room[self.opponent_agent_id],max_call=max_call)
		self.comm_counts += 1
	...
	def get_action_roco(self, observation, goal):
		"""
		:param observation: {"edges":[{'from_id', 'to_id', 'relation_type'}],
		"nodes":[{'id', 'category', 'class_name', 'prefab_name', 'obj_transform':{'position', 'rotation', 'scale'}, 'bounding_box':{'center','size'}, 'properties', 'states'}],
		"messages": [None, None]
		...
		if self.communication:
			for i in range(len(observation["messages"])):
				if observation["messages"][i] is not None:
					text= copy.deepcopy(observation["messages"][i])
					self.dialogue_history.append(f"{self.agent_names[i]}: {text}")
					match = re.search(r'<Exit>',text)
					if match:#find that the disscussion end. need to extract plan and parse it 
						if self.host:
							plan = re.search(r'alice_plan:\s*(.*?)\s*bob_plan:', text)
						else:
							plan = re.search(r'bob_plan:\s*(.*)', text)
						if plan:
							plan = plan.group(1)#TODO:check the parsing step
							plan = self.LLM.parse_answer(self.LLM.available_actions,plan)
							self.plan = plan
							self.episode_logger.info(f"{self.agent_id} agent plan : {plan}")
							self.action_history.append( f"{self.plan} at step {self.steps}")
						self.dialogue_history = []
		...
		if self.id_inside_room[self.opponent_agent_id] == self.current_room['class_name']:
			self.opponent_grabbed_objects = opponent_grabbed_objects
		if observation["disscuss"] == 1:
			if observation["call"] < 2:# check if the max call reach
				turns = observation["turns"]
				if turns == 1 - self.host:
					message = self.LLM_disscuss()
					self.episode_logger.info(
						f"agent{self.agent_id} disscuss:{message}"
					)
					action = f"[send_message] <{message}>"
				else:
					action = "[waiting]"
				return action,{}
				...
				if self.host:
					message = self.LLM_disscuss(max_call=True)
					self.episode_logger.info(
						f"agent{self.agent_id} disscuss:{message}"
					)
					action = f"[send_message] <{message}>"
		...
		while action is None:
			if self.plan is None:
				action = wait_for_disscussion()
				return action,{}# to avoid stuck justification
			if self.plan.startswith('[goexplore]'):
				action = self.goexplore()
			elif self.plan.startswith('[gocheck]'):
				action = self.gocheck()
