from LLM .LLM_capo_single import LLM
...
class LLM_agent:
	...
	def refiner(self):
		output = self.LLM.refine(self.current_room,[self.id2node[x] for x in self.grabbed_objects],self.satisfied,self.unchecked_containers,self.ungrabbed_objects,self.id_inside_room[self.goal_location_id],self.meta_plan)
		return output
	
	def parsing(self):
		output = self.LLM.parsing(self.current_room,[self.id2node[x] for x in self.grabbed_objects],self.satisfied,self.unchecked_containers,self.ungrabbed_objects,self.id_inside_room[self.goal_location_id],self.meta_plan,self.action_history)
		return output
		...
		while action is None:
			if self.plan is None:
				...
				if plan is None: # NO AVAILABLE PLANS! Explore from scratch!
					...
					plan = f"[wait]"
				self.plan = plan
				self.action_history.append(f"{plan} at {self.steps}")
