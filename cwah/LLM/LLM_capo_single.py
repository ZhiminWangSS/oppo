...
class LLM:
    def __init__(self,
        ...
        self.metal_plan_init_prompt = df["prompt"][0]
        self.refine_prompt = df["prompt"][1]
        self.parsing_prompt = df["prompt"][2]
        ...
        if self.source == 'openai' or self.source == 'aliyun':
    ...
    def progress2text(self, current_room, grabbed_objects, unchecked_containers, ungrabbed_objects, goal_location_room, satisfied,room_explored):
        sss = {}
    ...
    def meta_plan_init(self):
        prompt = self.metal_plan_init_prompt.replace('$GOAL$', self.goal_desc)
        chat_prompt = [{"role": "user", "content": prompt}]
        output,usage = self.generator(chat_prompt,self.sampling_params)
        return output[0]
    def refine(self,current_room, grabbed_objects, satisfied, unchecked_containers, ungrabbed_objects, goal_location_room, previou_metaplan,room_explored = None):
        progress_desc = self.progress2text(current_room, grabbed_objects, unchecked_containers, ungrabbed_objects, goal_location_room, satisfied,room_explored)
        prompt = self.refine_prompt.replace('$GOAL$', self.goal_desc)
        prompt = prompt.replace('$PROGRESS$', progress_desc)
        prompt = prompt.replace("$PREVIOUS\_PLAN$",previou_metaplan)
        chat_prompt = [{"role": "user", "content": prompt}]
        output,usage = self.generator(chat_prompt,self.sampling_params)
        return output[0]
    def parsing(self,current_room, grabbed_objects, satisfied, unchecked_containers, ungrabbed_objects, goal_location_room, previou_metaplan,action_history,room_explored = None):
        progress_desc = self.progress2text(current_room, grabbed_objects, unchecked_containers, ungrabbed_objects, goal_location_room, satisfied,room_explored)
        prompt = self.parsing_prompt.replace('$GOAL$', self.goal_desc)
        prompt = prompt.replace('$PROGRESS$', progress_desc)
        prompt = prompt.replace("$PREVIOUS\_PLAN$",previou_metaplan)
        action_history_desc = ", ".join(action_history[-10:] if len(action_history) > 10 else action_history)
        ...
        chat_prompt = [{"role": "user", "content": prompt}]
        output,usage = self.generator(chat_prompt,self.sampling_params)
        plan = self.parse_answer(available_plans_list,output[0])
        return plan
