...
class LLM_roco:
    ...
    def disscuss(self, current_room, grabbed_objects, satisfied, unchecked_containers, ungrabbed_objects, goal_location_room, action_history, dialogue_history, opponent_grabbed_objects, opponent_last_room, room_explored = None,max_call = None):
        ...
        try:
            ...
            print(e)
        action_history_desc = ", ".join(action_history[-10:] if len(action_history) > 10 else action_history)
        dialogue_history_desc = '\n'.join(dialogue_history[-6:] if len(dialogue_history) > 6 else dialogue_history)
        prompt = self.prompt_template.replace('$GOAL$', self.goal_desc)
