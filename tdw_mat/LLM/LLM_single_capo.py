...
#no opponent modeling
    ...
    def meta_plan_init(self,current_room, rooms_explored, holding_objects, object_list, obj_per_room):
        self.current_room = current_room
        self.rooms_explored = rooms_explored
        self.holding_objects = holding_objects
        self.object_list = object_list
        self.obj_per_room = obj_per_room
        prompt = self.meta_plan_prompt.replace('$GOAL$', self.goal_desc)
        chat_prompt = [{"role":"user","content":prompt}]
        output,usage = self.generator(chat_prompt,self.sampling_params)
        return output[0]
    
    def refine_meta_plan(self,current_step, current_room, rooms_explored, holding_objects, satisfied, object_list, obj_per_room, previous_metaplan):
        self.current_room = current_room
        self.rooms_explored = rooms_explored
        self.holding_objects = holding_objects
        self.object_list = object_list
        ...
        prompt = prompt.replace("$PROGRESS$",progress_desc)
        prompt = prompt.replace("$PREVIOUS\_PLAN$",previous_metaplan)
        chat_prompt = [{"role":"user","content":prompt}]
        output,usage = self.generator(chat_prompt,self.sampling_params)
        return output[0]
    def parsing(self,current_step, current_room, rooms_explored, holding_objects, satisfied, object_list, obj_per_room, previous_metaplan,action_history):
        self.current_room = current_room
        self.rooms_explored = rooms_explored
        self.holding_objects = holding_objects
        self.object_list = object_list
        self.obj_per_room = obj_per_room
        progress_desc = self.progress2text(current_step, satisfied)
        action_history_desc = ", ".join(action_history[-10:] if len(action_history) > 10 else action_history)#action history is a list
        ...
        output,usage = self.generator(chat_prompt,self.sampling_params)
        plan = output[0]
        plan = self.parse_answer(available_plans_list,plan)[0]
