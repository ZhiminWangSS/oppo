...
class LLM_roco:
    def __init__(self,
        ...
        self.total_cost = 0
        # for roco to record the available actions
    ...
    #TODO:change to adapt the Wait action
    ...
    def get_available_plans(self):#for roco
        """
        go to room {}
        ...
        available_plans.append("Wait")#for no available plans
        plans = ""
    ...
    def disscuss(self,current_step,current_room,rooms_explored,holding_objects,satisfied,object_list,obj_per_room,action_history,dialogue_history,opponent_grabbed_objects = None,opponent_last_room = None,max_call = NotImplementedError):
        ...
        progress_desc = self.progress2text(current_step, satisfied, opponent_grabbed_objects, opponent_last_room)
        action_history_desc = ", ".join(action_history[-10:] if len(action_history) > 10 else action_history)
        dialogue_history_desc = '\n'.join(dialogue_history[-6:] if len(dialogue_history) > 6 else dialogue_history)#history for one round
        available_plans, num, available_plans_list = self.get_available_plans()
        #record for roco
        self.available_actions = available_plans_list

        prompt = self.prompt_template.replace('$GOAL$', self.goal_desc)
        prompt = prompt.replace('$current_observation$',progress_desc)
        prompt = prompt.replace('$previous_actions$',action_history_desc)
        prompt = prompt.replace('$dialogue history$',dialogue_history_desc)
        prompt = prompt.replace('$available_actions$',available_plans)
        #reach the last call in one round
        if max_call == True:
            addition = f"""
This is the last call, you must end your response by choosing choice 2)
                    """
            prompt += addition
