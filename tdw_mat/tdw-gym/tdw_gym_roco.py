...
class TDW(Env):
    ...
    def step(self, actions):
        ...
        if self.disscuss_status[0] == 1 and self.disscuss_status[1] == 1:
                finish = True
        num_frames = 0
        #TODO:when one wait for disscussion the other agent may take many time to finish the current action
        while not finish: # continue until any agent's action finishes
            ...
            for replicant_id in self.controller.replicants:
                if self.controller.replicants[replicant_id].action.status == ActionStatus.ongoing and self.disscuss_status[1-replicant_id] == 1:
                    finish = False     
