...
class TDW(Env):
    def __init__(self, port = 1071, number_of_agents = 1, demo=False, rank=0, num_scenes = 0, train=False, \
        ...
        self.observation_space_single = gym.spaces.Dict({
            ...
            'oppo_held_objects_true':gym.spaces.Tuple((self.hand_object_space, self.hand_object_space)),# ground true oppo held objects
    ...
    def get_obs(self):
        ...
        for replicant_id in self.controller.replicants:
            ...
            oppo_held_objects_true = list(self.controller.state.replicants[1 - replicant_id].values())# ground true oppo held objects
            obs[id]['oppo_held_objects'] = []
            obs[id]['oppo_held_objects_true'] = []
            for hand in range(2):# get after seeing
            ...
            for hand in range(2):# ground truth holding
                if oppo_held_objects_true[hand] is None:
                    obs[id]['oppo_held_objects_true'].append({
                        'id': None,
                        'type': None,
                        'name': None,
                        'contained': [None, None, None],
                ...
                elif self.get_object_type(oppo_held_objects_true[hand]) == 0:
                    obs[id]['oppo_held_objects_true'].append({
                        'id': oppo_held_objects_true[hand],
                        'type': 0,
                        'name': self.object_names[oppo_held_objects_true[hand]],
                        'contained': [None, None, None],
                        'contained_name': [None, None, None],
                    })
                else:
                    if oppo_held_objects_true[hand] in self.containment_all.keys(): 
                        contained_obj = [x for x in self.containment_all[oppo_held_objects_true[hand]] if x not in oppo_held_objects_true and x in self.target_object_ids]
                        obs[id]['oppo_held_objects_true'].append({
                            'id': oppo_held_objects_true[hand],
                            'type': 1,
                            'name': self.object_names[oppo_held_objects_true[hand]],
